...
TopoRelGST_0::TopoRelGST_0(vector<vector<int>> &rutas, int cant_stops){
    cout << "Constructor TopoRelGST_0" << endl;
    n_stops = cant_stops;
    n_concat = 0;
    n_rutas = rutas.size();
...
*/
    ...
    sizePrevCompr = 0;
    gstStops = vector<bit_vector>(n_rutas, bit_vector(finSec, 0));
    for(int i = 0; i < n_rutas; i++){
        for(int j = 0; j < rutas[i].size(); j++){
            gstStops[i][rutas[i][j]] = 1;
        }
    }
...
bool TopoRelGST_0::tr_intersect(int x, int y){
