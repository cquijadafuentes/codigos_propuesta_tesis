...
if [ $# -ne 3 ]; then
	...
	echo "En cada folder deben existir los archivos <file>.gst6, <file>.queries.<i> y el archivo <file>.results.<i>"
	exit 1
fi
...
do
	...
	do
		...
		./TopoRel_GST_6_test_allContained_byQueries ${DATAFOLDER1}${filename}.gst6 ${DATAFOLDER1}${filename}.queries.${i} ${DATAFOLDER1}${filename}.results.${i} >> ${OUTPUTFILE}
	done
	echo " ----------------------------------------------------- " >> ${OUTPUTFILE}
done
...
do
	...
	do
		...
		./TopoRel_GST_6_test_allContained_byQueries ${DATAFOLDER2}${filename}.gst6 ${DATAFOLDER2}${filename}.queries.${i}  ${DATAFOLDER2}${filename}.results.${i} >> ${OUTPUTFILE}
	done
	echo " ----------------------------------------------------- " >> ${OUTPUTFILE}
done