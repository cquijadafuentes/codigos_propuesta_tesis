...
bool noIguales(bool a, bool b, bool c, bool d, bool e, bool f){
...
int main(int argc, char const *argv[]){
    ...
    TopoRelGST_4 tt_4(routes, n_stops);
    TopoRelNaivePreComp trpc(routes, n_stops);
    int contAgregadas = 0;
    int contBasicas = 0;

    string rNaive, rNaivePC, rGST_0, rGST_1, rGST_2, rGST_4;
    for(int i = 0; i < routes.size(); i++){
        for(int j = 0; j < routes.size(); j++){
            ...
            rGST_4 = tt_4.obtenerRelacion(i, j);
            // 8 Relaciones básicas 
            if(rNaive != rNaivePC || rNaivePC != rGST_0 || rGST_0 != rGST_1 || rGST_1 != rGST_2 || rGST_2 != rGST_4){
            	cout << "i: " << i << " - j: " << j << endl;
			...
			bool tt4With = tt_4.tr_within(i,j);
			...
			bool tt4Cont = tt_4.tr_contains(i, j);
			...
			bool tt4Inte = tt_4.tr_intersect(i, j);

            if(noIguales(trWith, trpcWith, tt0With, tt1With, tt2With, tt4With) ||
            		noIguales(trCont, trpcCont, tt0Cont, tt1Cont, tt2Cont, tt4Cont) ||
            		noIguales(trInte, trpcInte, tt0Inte, tt1Inte, tt2Inte, tt4Inte)){
            	cout << "\tNaive\tNaivePC\tGST_0\tGST_1\tGST_2\tGST_4" << endl;
            	cout << "With\t" << trWith << "\t" << trpcWith << "\t" << tt0With << "\t" << tt1With << "\t" << tt2With << "\t" << tt4With << endl;
            	cout << "Cont\t" << trCont << "\t" << trpcCont << "\t" << tt0Cont << "\t" << tt1Cont << "\t" << tt2Cont << "\t" << tt4Cont << endl;
            	cout << "Inte\t" << trInte << "\t" << trpcInte << "\t" << tt0Inte << "\t" << tt1Inte << "\t" << tt2Inte << "\t" << tt4Inte << endl;
